  ...
  private async req(route: String, requestBody: Partial<RESTBody> = {}, method: String): Promise<Record<any, any>> {
    ...
    if (requestBody.files) {
      ...
      for (let i = 0; i < requestBody.files.length; i++) {
        ...
        data.append(
          `files[${requestBody.files[i].key ?? i}]`,
          new Blob([requestBody.files[i].data]),
          requestBody.files[i].name as string,
        );
      }
    ...
    if (statusCode > 399 && statusCode < 600) {
      if (statusCode > 499 && statusCode != 503) {
        throw new RESTError(
          requestBody.body,
          '5xx',
          (requestBody.files as unknown as Array<File>) ?? [],
          await body.json(),
          this.route + route,
        );
      } else {
